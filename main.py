# ✅ تحديث كود المستخدم والمندوب لإضافة: إنهاء الطلب + التقييم
...
async def handle_user_role(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # التحقق من حالة المندوب عند التسجيل
    if txt == "🚚 مندوب":
        try:
            ...
            if row:
                return await update.message.reply_text("✅ مفعل." if row[0] else "⏳ في انتظار المراجعة.")
        except Exception as e:
            logging.error(e)
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_states.get(uid) == "awaiting_id_photo":
        ...
        try:
            ...
            await update.message.reply_text("✅ تم الاستلام، في انتظار المراجعة.")
        except Exception as e:
            logging.error(e)
            await update.message.reply_text("❌ فشل في الرفع أو الحفظ.")
        user_states[uid] = None; user_data[uid] = {}

# التعامل مع الأزرار (عرض السعر والزمن - إنهاء الطلب - التقييم)
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d.startswith("time_"):
        tm = d.split("_")[1]; info = user_data.get(uid, {}); oid = info.get("order_id"); pr = info.get("price")
        try:
            conn = get_conn(); cur = conn.cursor()
            ...
            logging.error(e); await q.message.reply_text("❌ فشل في إرسال العرض.")
        user_states[uid] = None; user_data[uid] = {}
    ...
    if d.startswith("done_"):
        ...
        try:
            ...
            user_id = cur.fetchone()[0]; conn.commit(); conn.close()
            ...
            await context.bot.send_message(chat_id=user_id, text="✅ تم التوصيل، قيّم المندوب:", reply_markup=kb)
        except Exception as e:
            logging.error(e)
            await q.message.reply_text("❌ فشل في إنهاء الطلب.")

    if d.startswith("rate_"):
        parts = d.split("_"); oid = int(parts[1]); stars = int(parts[2])
        try:
            conn = get_conn(); cur = conn.cursor()
            cur.execute("SELECT agent_id FROM offers WHERE order_id=%s", (oid,))
            agid = cur.fetchone()[0]
            cur.execute("INSERT INTO ratings (order_id, agent_id, rating) VALUES (%s,%s,%s)", (oid, agid, stars))
            conn.commit(); conn.close()
            await q.message.reply_text("✅ تم تسجيل تقييمك. شكرًا!")
        except Exception as e:
            logging.error(e)
            await q.message.reply_text("❌ فشل في تسجيل التقييم.")
